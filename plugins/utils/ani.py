...
async def get_anime_manga(search_str, search_type, _id: str=None, author="Pornhwa Hub | Manhwa 18+", author_url="https://t.me/adult_mangas):  # sourcery no-metrics
    ...
    if result["title"]["english"] is not None:
        alternative_names.append(result["title"]["native"])
        caption = f"<b>{result['title']['english']}</b>\n\t\t(<code>{result['title']['romaji']}</code>)\n"
    else:
    ...
    reply_markup = []
    if synopsis_link:
        reply_markup.append(InlineKeyboardButton("Synopsis", url=synopsis_link))
    reply_markup.append(InlineKeyboardButton("Read More", url=result['siteUrl']))
    reply_markup = InlineKeyboardMarkup([reply_markup])

    return caption, image, reply_markup
